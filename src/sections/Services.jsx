      ...
      <div className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'} pb-20 `}>
        <section id="services" className={`${darkMode ? 'dark bg-gray-800' : 'light bg-red-100'} lg:w-[95%] w-full h-fit m-auto rounded-xl flex flex-col justify-center items-start lg:px-20 px-6 py-20 gap-10 `}>
          <div className='flex flex-col justify-center items-start gap-4'>
            <h1 data-aos="zoom-in" className='text-red-500 dark:text-white text-[30px]'>OUR SERVICES</h1>
          ...
          <div id='service-box' className='grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>
            {
              service.map((item,index)=>(
                <div data-aos="zoom-in" data-aos-delay="200" key={index} className='bg-white dark:bg-black h-[350px] rounded-xl px-8 py-16 flex flex-col justify-center items-start gap-4 border-b-[5px] border-red-600 hover:bg-red-300 cursor-pointer '>
                  <div className='p-6 rounded-full bg-red-200'>
                    <item.icon className='text-red-600 size-10 cursor-pointer transform hover:scale-110 transition-transform duration-300'/>
                  </div>
                  ...
                  <button className='border-b-2 text-red-600 border-red-600 font-semibold p-0 dark:text-white hover:bg-red-200 rounded-md dark:hover:text-black cursor-pointer transform hover:scale-110 transition-transform duration-300' >READ MORE</button>
                </div>
              ))
            }
          </div>
